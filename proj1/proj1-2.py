...
if(L_max==L_min): # if unique luminance e.g. pure color
    ...
    # get the histogram of the pixels in window    
    # 102 because the hist will get a length of bins -1
    b=np.arange(102)
    # hist length 101, bins length 102
    hist,bins = np.histogram(L_window.flatten(),b)
    # f length 101
    f = hist.cumsum()
    # gray level is 101, number of pixels is (H2-H1)*(W2-W1)
    m = 101.0/((H2-H1)*(W2-W1))
    # need an array of length 101
    hist_tmp = np.zeros(101)
    # manually calculate the first one at 0
    hist_tmp[0] = f[0]*0.5*m
    
    # calculate the talbe, range doesn't include 101, goes from 1 to 100
    for i in range(1,101): 
        hist_tmp[i] = (f[i-1]+f[i])*0.5*m
        if(hist_tmp[i]>100.0):
            hist_tmp[i]=100.0
   
    # result of floor values, stored in a list of length 101
    # the indice corresponds to the original gray/luminance level        
    hist_floor = np.floor(hist_tmp)

    # histogram equalization for all the pixels
    for i in range(0, rows) :
        for j in range(0, cols) :
            L, u, v = Luv_img[i,j]
            L = round(L) # L to 0-100
            if(L>L_max):
                L=100.0
            elif(L<L_min):
                L=0.0
